...
RemoveCorrelatedVarsAndCombinedVars <- function(teamName,onDate){
  ...
  if(!file.exists(inputFileName)){
    ...
    directoryPathImpVars <- "D:/Study_Material/DataAnalytics/GIT/ODIs/Output_Data/2 - Model_Creation/MC_Output/"
    impVarsfileName <- paste(teamNameSpaceRemoved,"_Important_Variables",".csv",sep = "")
    impVarsinputFileName <- paste(directoryPathImpVars,"MC-",teamName,"/",impVarsfileName,sep = "")
    print(impVarsinputFileName)
